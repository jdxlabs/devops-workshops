...
resource "aws_instance" "nodejs_servers" {
  count                = "${var.instances_nodejs_servers["nb"]}"
  ami                  = "${data.aws_ami.debian.id}"
  instance_type        = "${var.instances_nodejs_servers["type"]}"
  iam_instance_profile = "${aws_iam_instance_profile.instance_profile.name}"
  key_name             = "${aws_key_pair.bastion_keypair.key_name}"

  root_block_device = {
    volume_size = "${var.instances_nodejs_servers["root_hdd_size"]}"
    volume_type = "${var.instances_nodejs_servers["root_hdd_type"]}"
  }

  ebs_block_device = {
    volume_size = "${var.instances_nodejs_servers["ebs_hdd_size"]}"
    volume_type = "${var.instances_nodejs_servers["ebs_hdd_type"]}"
    device_name = "${var.instances_nodejs_servers["ebs_hdd_name"]}"
  }
